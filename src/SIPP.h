...
public:
    ...
    Path findPath4Cost(const ConstraintTable& constraint_table); // return A path that minimizes collisions, breaking ties by cost
    Path findPath4Collision(const ConstraintTable& constraint_table); // return A path that minimizes collisions, breaking ties by cost
	...
	SIPP(const Instance& instance, int agent, bool pre_process=true):
...
private:
	...
	// typedef boost::unordered_map<SIPPNode*, VectorE<SIPPNode*>, SIPPNode::NodeHasher, SIPPNode::eqnode> hashtable_t;
	// robin_hood速度略微快一些，但对内存占用太大，适合小地图
	// typedef robin_hood::unordered_map<SIPPNode*, list<SIPPNode*>, SIPPNode::NodeHasher, SIPPNode::eqnode> hashtable_t;
	// typedef google::dense_hash_map<SIPPNode*, list<SIPPNode*>, SIPPNode::NodeHasher, SIPPNode::eqnode> hashtable_t;
	// typedef ankerl::unordered_dense::map<SIPPNode*, list<SIPPNode*>, SIPPNode::NodeHasher, SIPPNode::eqnode> hashtable_t;
