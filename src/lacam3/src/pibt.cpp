...
PIBTa::PIBTa(const InstanceLa *_ins, DistTable *_D, int seed, bool _flg_swap,
           Scatter *_scatter)
    : ins(_ins),
      MT(std::mt19937(seed)),
      N(ins->N),
      ...
      D(_D),
      NO_AGENT_LA(N),
      occupied_now(V_size, NO_AGENT_LA),
      occupied_next(V_size, NO_AGENT_LA),
...
bool PIBTa::set_new_config(const Config &Q_from, Config &Q_to,
                          const std::vector<int> &order)
{
  ...
  for (auto i = 0; i < N; ++i) {
    ...
    if (Q_to[i] != nullptr) {
      // vertex collision
      if (occupied_next[Q_to[i]->id] != NO_AGENT_LA) {
        success = false;
        break;
      }
      // swap collision
      auto j = occupied_now[Q_to[i]->id];
      if (j != NO_AGENT_LA && j != i && Q_to[j] == Q_from[i]) {
        success = false;
        break;
      }
  ...
  // cleanup
  for (auto i = 0; i < N; ++i) {
    occupied_now[Q_from[i]->id] = NO_AGENT_LA;
    if (Q_to[i] != nullptr) occupied_next[Q_to[i]->id] = NO_AGENT_LA;
  }

  return success;
}

bool PIBTa::funcPIBT(const int i, const Config &Q_from, Config &Q_to)
{
  ...
  auto swap_operation = [&]() {
    if (swap_agent != NO_AGENT_LA &&                 // swap_agent exists
        Q_to[swap_agent] == nullptr &&            // not decided
        occupied_next[Q_from[i]->id] == NO_AGENT_LA  // free
    ) {
      // pull swap_agent
  ...
  for (size_t k = 0; k < K + 1; ++k) {
    ...
    if (occupied_next[u->id] != NO_AGENT_LA) continue;

    const auto j = occupied_now[u->id];

    // avoid swap conflicts with constraints
    if (j != NO_AGENT_LA && Q_to[j] == Q_from[i]) continue;
    ...
    if (j != NO_AGENT_LA && u != Q_from[i] && Q_to[j] == nullptr &&
...
int PIBTa::is_swap_required_and_possible(const int i, const Config &Q_from,
                                        Config &Q_to)
{
  // agent-j occupying the desired vertex for agent-i
  const auto j = occupied_now[C_next[i][0]->id];
  if (j != NO_AGENT_LA && j != i &&  // j exists
...
bool PIBTa::is_swap_required(const int pusher, const int puller,
                            Vertex *v_pusher_origin, Vertex *v_puller_origin)
{
  ...
  while (D->get(pusher, v_puller) < D->get(pusher, v_pusher)) {
    ...
    for (auto u : v_puller->neighbor) {
      ...
      if (u == v_pusher ||
          (u->neighbor.size() == 1 && i != NO_AGENT_LA && ins->goals[i] == u)) {
        --n;
      } else {
        tmp = u;
      }
...
bool PIBTa::is_swap_possible(Vertex *v_pusher_origin, Vertex *v_puller_origin)
{
  // simulate pull
  auto v_pusher = v_pusher_origin;
  auto v_puller = v_puller_origin;
