...
{
  for (auto k = 0; k < PIBT_NUM; ++k) {
    pibts.emplace_back(new PIBTa(ins, D, k + seed, FLG_SWAP, scatter));
  }
}
...
Solution Planner::get_refined_plan(const Solution &plan)
{
  ...
  if (depth < 1 && plan.size() > 3 &&
    ...
    // recursive LaCAM
    auto ins_tmp =
        InstanceLa(ins->G, plan[get_random_int(MT_internal, 1, plan.size() - 2)],
                 ins->goals, N);
    ...
    auto planner_tmp =
        Planner(&ins_tmp, 0, &deadline_tmp, seed_refiner, D, depth + 1);
