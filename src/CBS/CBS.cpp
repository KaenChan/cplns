...
{
	// cout << "CBS::solve(double _time_limit, int _cost_lowerbound, int _cost_upperbound)" << endl;
	...
	while (!cleanup_list.empty() && !solution_found)
	{
		...
		while (foundBypass)
		{
			...
			bool skip0 = false;
			bool skip1 = false;
			if(search_engines.size() > 5) {
				if(rand() % 2 == 0)
					skip0 = true;
				else
					skip1 = true;
			}
			// cout << "  skip0 " << skip0 << " skip1 " << skip1 << endl;
			for (int i = 0; i < 2; i++)
			{
				// if(skip0 && i==0)
				// 	continue;
...
bool CBS::terminate(HLNode* curr)
{
	...
	{// found a solution
		...
		if (!validateSolution())
		{
			cout << "CBS Solution invalid!!!" << endl;
			printPaths();
			exit(-1);
		}
		if (screen > 0) // 1 or 2
	...
	if (runtime > time_limit || num_HL_expanded > node_limit) {  // time/node out
		cout << "CBS timeout!!! " << runtime << " > " << time_limit << endl;
		solution_cost = -1;
		solution_found = false;
		if (screen > 0)  // 1 or 2
			printResults();
...
bool CBS::generateRoot()
{
	...
	if (paths_found_initially.empty())
	{
		...
		for (auto i : agents)
		{
			...
			{
				cout << "CBS No path exists for agent " << i << endl;
                delete root;
				return false;
			}
            runtime = (double)(clock() - start) / CLOCKS_PER_SEC;
