...
bool ECBS::solve(double time_limit, int _cost_lowerbound) {
    cout << "ECBS::solve(double time_limit, int _cost_lowerbound)" << time_limit
         << " " << _cost_lowerbound 
         << " screen " << screen << endl;
    ...
    while (!cleanup_list.empty() && !solution_found) {
        ...
        printf("------------ 1002 \n");
        if (terminate(curr) || phase_collision_asynch_interrupt->load() ||
        printf("------------ 1003 \n");
            phase_cost_asynch_interrupt->load()) {
            if (solution_found) goal_node = curr;
        ...
        printf("------------ 1005 \n");
            ...
            bool succ = heuristic_helper.computeInformedHeuristics(
                *curr, min_f_vals, time_limit - runtime);
            runtime = (double)(clock() - start) / CLOCKS_PER_SEC;
        ...
        if(num_HL_expanded % 10 == 0 || screen > 0)
            printf("num_HL_expanded %d\n", num_HL_expanded);
                ...
                for (int i = 0; i < 2; i++) {
                    ...
                    if (!solved[i]) {
                        ...
                        continue;
                             ...
                             child[i]->sum_of_costs <=
                                 suboptimality * cost_lowerbound &&
                             child[i]->distance_to_go <
                                 curr->distance_to_go)  // Bypass1
                    {
                        foundBypass = true;
                        for (const auto& path : child[i]->paths) {
                            /*if (path.second.first.size() !=
    path_copy[path.first]->size()) // CBS bypassing
    {
        foundBypass = false;
        break;
    }*/
                ...
                if (foundBypass) {
                    ...
                    classifyConflicts(
                        *curr);  // classify the new-detected conflicts
                } else {
                    for (int i = 0; i < 2; i++) {
                        if (solved[i]) {
                            pushNode(child[i]);
        ...
        if (!curr->children.empty())
            heuristic_helper.updateOnlineHeuristicErrors(
                *curr);  // update online heuristic errors
        curr->clear();
    }  // end of while loop
...
void ECBS::adoptBypass(ECBSNode* curr, ECBSNode* child,
...
bool ECBS::generateRoot() {
    ...
    // // 用了omp后会出错
    // const ECBSNode& root_node = *root;
    // const vector<ConstraintTable> &initial_constraints_const = initial_constraints;
    // const vector<Path*>& paths_const = paths;
    // double t1 = getTime();
    // #pragma omp parallel for num_threads(32)
    // for (int idx = 0; idx < num_of_agents; idx++) {
    //     int i = agents[idx];
    //     paths_found_initially[i] = search_engines[i]->findSuboptimalPath(
    //         // root_node, initial_constraints_const[i], paths_const, i, 0, suboptimality);
    //         *root, initial_constraints[i], paths, i, 0, suboptimality);
    // }
    ...
    for (auto i : agents) {
        cnt++;
        if(cnt % 1000 == 0)
            printf("generate root %d\n", cnt);
        paths_found_initially[i] = search_engines[i]->findSuboptimalPath(
            *root, initial_constraints[i], paths, i, 0, suboptimality);
        if (paths_found_initially[i].first.empty()) {
            cerr << "ECBS No path exists for agent " << i << endl;
            delete root;
            return false;
        }
        runtime = (double)(clock() - start) / CLOCKS_PER_SEC;
        if (runtime > time_limit) {
            cout << "Time out when generating the root CT node " << runtime
                 << endl;
            delete root;
            return false;
        }
    ...
    root->h_val = 0;
    root->depth = 0;
    printf("Root findConflicts\n");
    findConflicts(*root);
    printf("Root computeQuickHeuristics\n");
...
bool ECBS::findPathForSingleAgent(ECBSNode* node, int ag) {
    clock_t t = clock();
    auto new_path = search_engines[ag]->findSuboptimalPath(
        *node, initial_constraints[ag], paths, ag, min_f_vals[ag],
        suboptimality);
...
inline void ECBS::pushNode(ECBSNode* node) {
    ...
    switch (solver_type) {
        case high_level_solver_type::ASTAREPS:  // cleanup_list is called
                                                // open_list in ECBS
        ...
        case high_level_solver_type::EES:
            ...
            if (node->getFHatVal() <=
                suboptimality * inadmissible_cost_lowerbound)
                node->focal_handle = focal_list.push(node);
            break;
        default:
            break;
...
ECBSNode* ECBS::selectNode() {
    ...
    switch (solver_type) {
        case high_level_solver_type::EES:
            ...
            if (screen > 1 && cleanup_list.top()->getFVal() > cost_lowerbound)
                cout << "Lowerbound increases from " << cost_lowerbound
                     << " to " << cleanup_list.top()->getFVal() << endl;
            cost_lowerbound =
                max(cleanup_list.top()->getFVal(), cost_lowerbound);
            if (focal_list.top()->sum_of_costs <=
                suboptimality * cost_lowerbound) {  // return best d
                curr = focal_list.top();
                curr->chosen_from = "focal";
                /*curr->f_of_best_in_cleanup = cleanup_list.top()->getFVal();
                curr->f_hat_of_best_in_cleanup =
                cleanup_list.top()->getFHatVal(); curr->d_of_best_in_cleanup =
                cleanup_list.top()->distance_to_go; curr->f_of_best_in_open =
                open_list.top()->getFVal(); curr->f_hat_of_best_in_open =
                open_list.top()->getFHatVal(); curr->d_of_best_in_open =
                open_list.top()->distance_to_go; curr->f_of_best_in_focal =
                focal_list.top()->getFVal(); curr->f_hat_of_best_in_focal =
                focal_list.top()->getFHatVal(); curr->d_of_best_in_focal =
                focal_list.top()->distance_to_go; */
                focal_list.pop();
                cleanup_list.erase(curr->cleanup_handle);
                open_list.erase(curr->open_handle);
            } else if (open_list.top()->sum_of_costs <=
                       suboptimality * cost_lowerbound) {  // return best f_hat
                curr = open_list.top();
        ...
        case high_level_solver_type::ASTAREPS:
            ...
            if (cleanup_list.top()->getFVal() > cost_lowerbound) {
                if (screen == 3) {
                    cout << "  Note -- FOCAL UPDATE!! from |FOCAL|="
                         << focal_list.size()
                         << " with |OPEN|=" << cleanup_list.size()
                         << " to |FOCAL|=";
                }
                double old_focal_list_threshold =
                    suboptimality * cost_lowerbound;
                cost_lowerbound =
                    max(cost_lowerbound, cleanup_list.top()->getFVal());
                double new_focal_list_threshold =
                    suboptimality * cost_lowerbound;
                for (auto n : cleanup_list) {
                    if (n->sum_of_costs > old_focal_list_threshold &&
                        n->sum_of_costs <= new_focal_list_threshold)
                        n->focal_handle = focal_list.push(n);
                }
                if (screen == 3) {
        ...
        case high_level_solver_type::NEW:
            ...
            if (focal_list.empty())  // choose best f in the cleanup list (to
                                     // improve the lower bound)
            {
                ...
                curr->f_hat_of_best_in_cleanup =
                cleanup_list.top()->getFHatVal(); curr->d_of_best_in_cleanup =
                cleanup_list.top()->distance_to_go;*/
                cleanup_list.pop();
            } else  // choose best d in the focal list
            {
                ...
                curr->f_hat_of_best_in_cleanup =
                cleanup_list.top()->getFHatVal(); curr->d_of_best_in_cleanup =
                cleanup_list.top()->distance_to_go; curr->f_of_best_in_focal =
                focal_list.top()->getFVal(); curr->f_hat_of_best_in_focal =
                focal_list.top()->getFHatVal(); curr->d_of_best_in_focal =
                focal_list.top()->distance_to_go;*/
                focal_list.pop();
    ...
    if (screen > 1) cout << endl << "Pop " << *curr << endl;
    return curr;
}
...
void ECBS::printPaths() const {
    for (int i = 0; i < num_of_agents; i++) {
        cout << "Agent " << i << " ("
             << paths_found_initially[i].first.size() - 1 << " -->"
             << paths[i]->size() - 1 << "): ";
...
void ECBS::classifyConflicts(ECBSNode& node) {
    if (node.unknownConf.empty()) return;
    ...
    while (!node.unknownConf.empty()) {
        ...
        if (PC)
            if (node.chosen_from == "cleanup" ||
                ...
                (int)paths[a2]->size() - 1 ==
                    min_f_vals[a2])  // the path of at least one agent is its
                                     // shortest path
                computeConflictPriority(con, node);

        /*if (con->priority == conflict_priority::CARDINAL &&
        heuristic_helper.type == heuristics_type::ZERO)
        {
        ...
        /*if (mutex_reasoning)
        {
                // TODO mutex reasoning is per agent pair, don't do duplicated
        work... auto mdd1 = mdd_helper.getMDD(node, a1, paths[a1]->size()); auto
        mdd2 = mdd_helper.getMDD(node, a2, paths[a2]->size());

                auto mutex_conflict = mutex_helper.run(a1, a2, node, mdd1,
        mdd2);
            ...
            type == constraint_type::VERTEX)  // vertex conflict
        {
            ...
            if (rectangle != nullptr) {
                if (!PC) rectangle->priority = conflict_priority::UNKNOWN;
    ...
    if (type == constraint_type::EDGE)  // Edge conflict
    {
        if (timestep < (int)mdd1->levels.size()) {
            cardinal1 = mdd1->levels[timestep].size() == 1 &&
                        mdd1->levels[timestep].front()->location ==
                            paths[a1]->at(timestep).location &&
                        mdd1->levels[timestep - 1].size() == 1 &&
                        mdd1->levels[timestep - 1].front()->location ==
                            paths[a1]->at(timestep - 1).location;
        }
        if (timestep < (int)mdd2->levels.size()) {
            cardinal2 = mdd2->levels[timestep].size() == 1 &&
                        mdd2->levels[timestep].front()->location ==
                            paths[a2]->at(timestep).location &&
                        mdd2->levels[timestep - 1].size() == 1 &&
                        mdd2->levels[timestep - 1].front()->location ==
                            paths[a2]->at(timestep - 1).location;
        }
