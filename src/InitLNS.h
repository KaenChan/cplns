...
class InitLNS : public BasicLNS {
   public:
    ...
    int iter_of_run_after_sa_accept = 0;
    ...
    InitLNS(const Instance& instance, vector<Agent>& agents, double time_limit,
            ...
            int neighbor_size, int screen);

    bool getInitialSolution();
    bool getInitialSolutionMethod2();
    bool getInitialSolutionMethod3();
    bool run();
    bool runWait();
    ...
    string getSolverName() const override {
        return "InitLNS(" + replan_algo_name + ")";
    }
   ...
   private:
    ...
    bool updateCollidingPairs(set<pair<int, int>>& colliding_pairs,
