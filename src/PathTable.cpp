...
void PathTable::deletePath(int agent_id, const Path& path)
{
    ...
    for (int t = 0; t < (int)path.size(); t++)
    {
        // std::cout << " " << table[path[t].location].size() << " > "  << t 
        //     << " && " << table[path[t].location][t] << " == " << agent_id << std::endl;
...
// static std::map<int, int> table_stat;
...
void PathTableWC::insertPath(int agent_id, const Path& path)
{
    ...
    for (int t = 0; t < (int)path.size(); t++)
    {
        ...
        // int sz = table[path[t].location][t].size();
        // if (table_stat.find(sz) == table_stat.end())
        //     table_stat[sz] = 1;
        // else
        //     table_stat[sz]++;
        // table_cnt++;
        // if(table_cnt % 10 == 0)
        //     for(auto a : table_stat)
        //         printf("pathtablewc %d %d\n", a.first, a.second);
    }
    assert(goals[path.back().location] == MAX_TIMESTEP);
    goals[path.back().location] = (int) path.size() - 1;
    makespan = max(makespan, (int) path.size() - 1);
