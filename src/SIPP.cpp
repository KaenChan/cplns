...
Path SIPP::findPath(const ConstraintTable& constraint_table)
{
    ...
    if (constraint_table.path_table_for_CT != nullptr and !constraint_table.path_table_for_CT->table.empty())
        return findPath4Cost(constraint_table);
    else
        return findPath4Collision(constraint_table);
}

// find path by A*
// Returns a path that minimizes the collisions with the paths in the path table, breaking ties by the length
Path SIPP::findPath4Cost(const ConstraintTable& constraint_table)
{
    reset();
    //Path path = findNoCollisionPath(constraint_table);
    //if (!path.empty())
    //    return path;
    ReservationTable4Cost reservation_table(constraint_table, goal_location);
    Path path;
    ...
    // int sum = 0 ;
    // for(auto l : reservation_table.sit)
    //     sum += l.size();
    // cout 
    //     << "allNodes_table " << allNodes_table.size()
    //     << " useless_nodes " << useless_nodes.size()
    //     << " sit " << sum
    //     << " open " << open_list.size()
...
Path SIPP::findPath4Collision(const ConstraintTable& constraint_table)
{
	vector<int> &my_heuristic = *p_my_heuristic;
    reset();
    //Path path = findNoCollisionPath(constraint_table);
